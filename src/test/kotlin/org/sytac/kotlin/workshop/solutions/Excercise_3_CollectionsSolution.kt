...
object Excercise_3_CollectionsSolution {
    ...
    fun generatePairs(): List<Pair<Int, UUID>> =
        (0..5)
            .map { Random.nextInt(from = 100, until = 500) }
            .sorted()
            .map { it to randomUUID() }
...
//    zipped.forEach { (word, length) -> println("Word: $word, Length: $length") }
    ...
    fun `generate squares of even numbers between 1 and 10 then take if not empty also println all results`() {
        ...
        @Test
        fun `generate numbers from 0 until 11 skipping every 2 element (0,2,4,6,8,10), then zip it with animals sorted list, then filter keys taking only those below 6, and then map each key to string `() {
        ...
        fun `analyze weekday durations`() {
            ...
            val weekdays = durations.filterKeys { it != DayOfWeek.SATURDAY}.filterKeys { it != DayOfWeek.SUNDAY }
